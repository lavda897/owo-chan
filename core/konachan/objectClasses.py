class KonachanPost:
    """
    The data structure for images on Konachan. By default, all items are none,
    they will only be something else if Konachan specifies a value.
    ...
    md5 = None  # The MD5 hash of the post, i have no idea why its necessary to expose, but Konachan generates it
    creator_ID = None  # The post author's ID
    rating = None  # The rating of the post
    id = None # The post ID
    source = None # The post source
    ...
    def parse(self, post):
        ...
        self.rating = str(post['@rating'])  # The rating of the post
        self.id = int(post['@id'])  # The post ID
        self.source = str(post['@source']) # The post source
